...
| `TrackingArmer`, `InvalidationListener` | `src/RedisNearCache/Tracking/` | `RedisNearCacheConnection` |
...
Invalidation events call `L1.Remove(key)` and `inflight.MarkInvalidated(key)`. `FlushAll` and any `Armed`
event with a non-Initial reason call `L1.Clear()`.
