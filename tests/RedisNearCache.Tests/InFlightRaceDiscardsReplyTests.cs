...
/// <summary>
/// Needs access to the internal <see cref="RedisNearCacheOptions.TestHooks"/> hook, captured from the
/// configure callback at registration time, so it builds its own provider instead of using
/// <see cref="StandaloneCacheFixture"/>.
/// </summary>
public class InFlightRaceDiscardsReplyTests : IAsyncLifetime
{
    ...
    public async Task InFlightRaceDiscardsReply()
    {
        ...
        // The Redis reply for this read ("v1") arrives, but the hook above invalidates the key while the
        // read is in flight; the reply must be discarded rather than stored.
    ...
    {
        var key = TestHelpers.Key("evict-race");
        await _cache.SetAsync(key, "v1");
